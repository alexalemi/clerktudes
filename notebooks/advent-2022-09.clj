...
(def data-string (slurp "input/advent-2022-09.txt"))
(def test-string "R 4
U 4
L 3
D 1
...
(defn simple-move [[head tail] direction]
  (let [new-head (target direction head)
        new-tail (if (> (inf-norm new-head tail) 1) head tail)]
    [new-head new-tail]))

(defn simple-total-visited [instructions initial]
  (->> (expand instructions)
       (reductions simple-move initial)
       (map peek)
       (into #{})
       count))
...
(test/deftest test-part-1
  (test/is (= 13 (simple-total-visited test-data short-rope))))

(def ans1 (simple-total-visited data short-rope))
